...
public class RandomOptimizer {
    ...
    public Operator getOptimizedPlan() {
        ...
        /**
         * Randomly restart the gradient descent until * the maximum specified number of random restarts
         * (NUMITER) * has satisfied
         */
        for (int j = 0; j < NUMITER; j++) {
            ...
            if (numJoin != 0) {
                ...
                while (flag) { // flag = false when local minimum is reached
                    ...
                    /**
                     * In this loop we consider from the * possible neighbors (randomly selected) * and take
                     * the minimum among for next step
                     */
    ...
    protected Operator neighborAssoc(Operator root, int joinNum) {
        ...
        if (left.getOpType() == OpType.JOIN && right.getOpType() != OpType.JOIN) {
            ...
            if (RandNumb.flipCoin()) transformLefttoRight(op, (Join) left);
            else transformRighttoLeft(op, (Join) right);
        } else {
    ...
    protected void transformLefttoRight(Join op, Join left) {
        ...
        /**
         * CASE 1 : ( A X a1b1 B) X b4c4 C = A X a1b1 (B X b4c4 C) * a1b1, b4c4 are the join conditions
    ...
    protected void transformRighttoLeft(Join op, Join right) {
        ...
        Join temp;
        /**
         * CASE 3 : A X a1b1 (B X b4c4 C) = (A X a1b1 B ) X b4c4 C * a1b1, b4c4 are the join conditions
         * at that join operator
         */
    ...
    protected Operator findNodeAt(Operator node, int joinNum) {
        if (node.getOpType() == OpType.JOIN) {
            if (((Join) node).getNodeIndex() == joinNum) {
                ...
                if (temp == null) temp = findNodeAt(((Join) node).getRight(), joinNum);
                return temp;
            }
